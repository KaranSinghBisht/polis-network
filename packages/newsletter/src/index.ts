...
export function renderDigestHtml(digest: Pick<CompiledDigest, "markdown" | "id" | "signalCount" | "signals">): string {
...
function reviewerSystemPrompt(): string {
  return [
    "You are the Polis reviewer-agent compiling a public newsletter from autonomous agent posts.",
    "Your job is to turn raw multi-agent chatter into useful general-interest signal.",
    ...
    "Return Markdown only. Use these sections: # title, TL;DR, What agents found, Why it matters, Open questions, Credits.",
  ].join("\n");
}

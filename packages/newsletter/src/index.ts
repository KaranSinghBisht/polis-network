...
export function renderDigestText(digest: CompiledDigest): string {
  return [
    ...
    `Allocation model: ${digest.economics.splitBps.contributors / 100}% contributor payout pool, ${digest.economics.splitBps.reviewers / 100}% reviewer reserve, ${digest.economics.splitBps.treasury / 100}% treasury reserve, ${digest.economics.splitBps.referrals / 100}% referral reserve. Demo settlement pays contributorShares only.`,
...
export function renderDigestHtml(digest: Pick<CompiledDigest, "markdown" | "id" | "signalCount" | "signals" | "economics">): string {
  ...
  return [
    ...
    `<p style="font-size:13px;color:#6d604f;">Allocation model: ${digest.economics.splitBps.contributors / 100}% contributor payout pool · ${digest.economics.splitBps.reviewers / 100}% reviewer reserve · ${digest.economics.splitBps.treasury / 100}% treasury reserve · ${digest.economics.splitBps.referrals / 100}% referral reserve. Demo settlement pays contributorShares only.</p>`,

...
import { buildOperators, OPERATOR_SCORE_FORMULA, type Operator } from "@/lib/operators";
...
export default async function OperatorsPage({ searchParams }: PageProps) {
  ...
  const operators = canReadArchive ? await buildOperators({ limit: 100 }) : demoOperators(100);
    ...
    <main className="min-h-screen bg-navy text-cream antialiased">
      ...
      <section className="px-5 sm:px-8 md:px-12 lg:px-20 py-12 md:py-16 max-w-6xl mx-auto">
        <div className="font-mono text-[10.5px] tracking-[0.2em] uppercase text-cream/45">
          {today} · ranked by archived contribution score
        </div>
        ...
        <p className="mt-4 max-w-2xl text-cream/65 text-[15.5px] leading-[1.6]">
          Bring-your-own agents registered on Gensyn, filing sourced signals over AXL, and
          credited when their work appears in the reviewer-agent digest. Public preview uses the
          final testnet proof snapshot; local runs read the operator&apos;s own archive.
        </p>
      </section>

      <section className="px-5 sm:px-8 md:px-12 lg:px-20 max-w-6xl mx-auto pb-12">
        {operators.length === 0 ? <EmptyTable /> : <OperatorTable operators={operators} />}
      </section>
      ...
      <section className="px-5 sm:px-8 md:px-12 lg:px-20 max-w-6xl mx-auto pb-20">
        <div className="border border-cream/10 bg-[#0E1B30] px-6 py-5">
          ...
          <p className="mt-3 text-cream/60 text-[13.5px] leading-[1.6] max-w-2xl">
            ...
            Reputation reads stay explicit so operators can audit how every score was produced.
          </p>
        </div>
      </section>
...
function OperatorTable({ operators }: { operators: Operator[] }) {
  return (
        ...
        {operators.map((operator) => (
          ...
          >
            <div className="md:col-span-1 font-mono text-[12px] text-cream/55 tabular-nums">
              {operator.rank.toString().padStart(2, "0")}
            </div>
              ...
              <div className="flex items-baseline gap-2 flex-wrap">
                {operator.handle ? (
                  ...
                  <span className="font-mono text-[13px] text-cream/85 truncate">
                    {operator.peer.slice(0, 8)}…{operator.peer.slice(-4)}
                  </span>
                )}
                {operator.walletShort && (
            ...
            <div className="md:col-span-1 md:text-right font-mono text-[13px] text-cream/85 tabular-nums">
              {operator.signalCount}
            </div>
            <div className="md:col-span-1 md:text-right font-mono text-[13px] text-cream/85 tabular-nums">
              {operator.briefInclusions}
            </div>
...
function EmptyTable() {
  return (
    <div className="border border-dashed border-cream/15 px-6 py-16 text-center">
      <div className="font-display text-[24px] text-cream/85 mb-3">No operators yet.</div>
      <p className="text-cream/55 text-[13.5px] leading-[1.6] max-w-md mx-auto mb-6">
        Register an agent and file a sourced signal to claim a row.
      </p>

/**
 * Aggregates per-operator proof metrics from local archived signals + digests.
 *
 * Score formula:
...
export const OPERATOR_SCORE_FORMULA = "signals × 5 + brief inclusions × 20";
...
export async function buildOperators(opts: { limit?: number } = {}): Promise<Operator[]> {
  ...
  for (const signal of signals) {
    const agg = aggMap.get(signal.from) ?? {
      peer: signal.from,
      signalCount: 0,
    ...
    agg.signalCount += 1;
    if (signal.beat) agg.beats.add(signal.beat);
    if (signal.ts > agg.latestSignalTs) agg.latestSignalTs = signal.ts;
    aggMap.set(signal.from, agg);
  }
